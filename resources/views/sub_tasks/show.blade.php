              ...
              <div class="lg:w-1/2 md:w-2/3 mx-auto flex flex-wrap -m-2">
                ...
                <form method="post" action="{{ route('tasks.sub_tasks.update', [
                  ...
                  <div class="p-2 w-full">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') ?? $subTask->name }}"
                  ...
                  <div class="p-2 w-full">
                    ...
                    <textarea id="content" name="content"
                      class="w-full bg-gray-100 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2
                        focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out">{{ old('content') ?? $subTask->content }}</textarea>
                  </div>
                  <div class="flex w-full justify-between items-center mt-6">
                    {{-- 戻るボタン（左側） --}}
                    <a href="{{ route('tasks.show', ['project_id' => $subTask->task->project->id, 'task_id' => $subTask->task->id]) }}"
